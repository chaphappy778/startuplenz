...
export default function SignupPage() {
  ...
  async function handleGoogleSignup() {
    setError(null);
    // Match login: stash a `next` cookie so /auth/callback can fall back to it
    // when Supabase drops query strings from redirectTo.
    document.cookie = `oauth_next=${encodeURIComponent("/")}; max-age=600; path=/; SameSite=Lax`;
    const supabase = createClient();
      ...
      <div className="auth-shell">
        ...
        <div className="auth-card">
          ...
          <p className="auth-fine-print">
            ...
            <Link href="/terms" className="auth-link auth-link-muted">Terms</Link>
            {" "}and{" "}
            <Link href="/privacy" className="auth-link auth-link-muted">Privacy Policy</Link>.
          </p>
        </div>
